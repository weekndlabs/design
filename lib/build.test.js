...
test('every unthemed token reaches the CSS', async () => {
  const unthemed = (await loadScale())
    // Two groups do not belong in the core, and both have a test of their own
    // asserting their absence. Container widths cannot be CSS variables at all,
    // because @container cannot read one. Archetype tokens belong to
    // marketing.css and catalog.css, which is what makes them archetypes.
    .filter(({ path }) => path[0] !== 'container' && path[0] !== 'archetype')
...
test('no archetype token reaches the core', async () => {
  // The whole contract of an archetype: it may add tokens, it may not change
  // what every product gets. A leak here would put a 30px section radius and a
  // hero gradient into an app that only imported tokens.css.
  const names = (await loadScale())
    .filter(({ path }) => path[0] === 'archetype')
    .map(({ path }) => `--wl-${path.slice(2).join('-')}`);
  assert.ok(names.length >= 6, `only ${names.length} archetype tokens found, the loader is wrong`);
  for (const name of names) {
    assert.doesNotMatch(css, new RegExp(`${name}\\s*:`), `${name} leaked into tokens.css`);
  }
});

test('each archetype defines its own tokens and nothing else', async () => {
  ...
  const scale = await loadScale();
  const owned = (group) =>
    scale.filter(({ path }) => path[0] === 'archetype' && path[1] === group)
      .map(({ path }) => `--wl-${path.slice(2).join('-')}`);

  for (const [name, sheet, group] of [['marketing', marketing, 'marketing'], ['catalog', catalog, 'catalog']]) {
    assert.match(sheet, new RegExp(`\\.wl-${name} \\{`), `${name}.css has no scope block`);
    for (const token of owned(group)) {
      assert.match(sheet, new RegExp(`${token}:`), `${name}.css is missing ${token}`);
    }
    // The other archetype's tokens must not appear, or the two are one sheet
    // with extra steps.
    for (const token of owned(group === 'marketing' ? 'catalog' : 'marketing')) {
      assert.doesNotMatch(sheet, new RegExp(`${token}:`), `${name}.css defines ${token}, which is not its own`);
    }
  }
});

test('archetype container queries are emitted last', () => {
  // They tie with the rules they are meant to beat on specificity, so source
  // order decides. This already shipped as a bug once, on the docs site nav.
  for (const file of ['marketing.css', 'catalog.css']) {
    const sheet = readFileSync(new URL(`../dist/${file}`, import.meta.url), 'utf8');
    const query = sheet.indexOf('@container');
    assert.ok(query > 0, `${file} has no container query`);
    const after = sheet.slice(query).replace(/@container[^{]*\{[\s\S]*?\n\}/g, '').trim();
    assert.equal(after, '', `${file} has rules after its container query, which will beat it`);
  }
});

test('the declarations cover every export and every role', () => {
  // Generated beside the values, so a role added to ROLES is typed the moment
  // it exists. Hand-written types would be a second list kept in step by
  // memory, and the failure is silent: a consumer's typo resolves to undefined.
  const dts = readFileSync(new URL('../dist/tokens.d.ts', import.meta.url), 'utf8');
  for (const name of ['THEMES', 'ROLES', 'TERMINAL', 'TEXT_ROLES', 'themes', 'terminal', 'containers']) {
    assert.match(dts, new RegExp(`export declare const ${name}:`), `${name} is exported untyped`);
  }
  for (const role of ROLES) assert.ok(dts.includes(`'${role}'`), `Role union is missing ${role}`);
  for (const role of TERMINAL) assert.ok(dts.includes(`'${role}'`), `TerminalRole union is missing ${role}`);
  for (const theme of THEMES) assert.ok(dts.includes(`'${theme}'`), `Theme union is missing ${theme}`);

  // Every runtime export needs a declaration, or a consumer gets `any` for it
  // and the types are decoration.
  const js = readFileSync(new URL('../dist/tokens.js', import.meta.url), 'utf8');
  for (const [, name] of js.matchAll(/export const (\w+)/g)) {
    assert.match(dts, new RegExp(`\\b${name}\\b`), `${name} is exported by tokens.js and absent from tokens.d.ts`);
  }
});

test('every export the package declares resolves to a file that exists', () => {
  // An exports map that points at a missing file fails at import time in the
  // consumer, not here, and only for the entry point nobody tested.
  const pkg = JSON.parse(readFileSync(new URL('../package.json', import.meta.url), 'utf8'));
  for (const [entry, target] of Object.entries(pkg.exports)) {
    const paths = typeof target === 'string' ? [target] : Object.values(target);
    for (const path of paths) {
      if (path.includes('*')) continue; // the fonts glob
      assert.ok(
        existsSync(new URL(`../${path}`, import.meta.url)),
        `exports["${entry}"] points at ${path}, which does not exist`
      );
    }
  }
});

test('the Figma export covers every role, with a mode per theme', () => {
  // Figma models a theme as modes inside one collection. Emitting a collection
  // per theme instead gives a designer two `background` variables and no way to
  // switch, which is the shape this is easiest to get wrong in.
  const figma = JSON.parse(readFileSync(new URL('../dist/tokens.figma.json', import.meta.url), 'utf8'));
  for (const role of ROLES) {
    if (role === 'radius') continue;
    const token = figma.color[role];
    assert.ok(token, `the Figma export is missing ${role}`);
    const modes = token.$extensions['com.figma'].modes;
    assert.deepEqual(Object.keys(modes), [...THEMES], `${role} has the wrong modes`);
    for (const theme of THEMES) {
      assert.equal(modes[theme], themes[theme][role], `${role} in ${theme} has drifted from the tokens`);
    }
  }
  for (const role of TERMINAL) assert.ok(figma.terminal[role], `the Figma export is missing terminal-${role}`);
});
