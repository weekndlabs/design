...
test('one host is named once, and canonical and sitemap both derive from it', () => {
  ...
  //
  // The fix is structural: `site` in astro.config.mjs is the only place the host
  // appears, Page.astro reads it through Astro.site, and @astrojs/sitemap
  // generates from the same value. Hardcoding it again is what this catches.
  ...
  assert.match(config, /integrations: \[sitemap\(\)\]/, 'the sitemap must be generated');
...
test('the docs host serves a robots.txt of its own', () => {
  ...
  assert.match(robots, /^Sitemap: https:\/\/\S+sitemap-index\.xml$/m, 'no Sitemap directive');
