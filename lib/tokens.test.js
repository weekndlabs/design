...
test('themes are named for modes and materials, with no brand prefix left', () => {
  assert.deepEqual(THEMES, ['light', 'dark', 'paper']);
  for (const theme of THEMES) {
    assert.doesNotMatch(theme, /night|receipt/, `${theme} still carries the old name`);
  }
});
...
test('the derived pair list covers every foreground role', () => {
  assert.equal(pairs.length, 14, `${pairs.length} pairs derived, the naming broke`);
