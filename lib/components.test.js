...
const prose = readFileSync(new URL('../dist/prose.css', import.meta.url), 'utf8');

const sheets = { 'components.css': css, 'marketing.css': marketing, 'catalog.css': catalog, 'prose.css': prose };
...
test('the component layer defines no token of its own', async () => {
  ...
  for (const [name, sheet] of Object.entries(sheets)) {
    if (name === 'marketing.css' || name === 'catalog.css') continue; // archetypes define theirs on purpose
