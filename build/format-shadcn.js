...
export const formatShadcn = ({ dictionary }) => {
  ...
  const containers = dictionary.allTokens
    ...
    .join('\n');
  // ground is the DEFAULT, so `bg-terminal` reads the way `bg-card` does and the
  // suffixed names are left for the palette.
  const terminal = TERMINAL.map((role) =>
    `  --color-terminal${role === 'ground' ? '' : `-${role}`}: var(--wl-terminal-${role});`).join('\n');
